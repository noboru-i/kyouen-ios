...
- (NSString *)getStageData:(NSInteger)currentMaxStageNo
                  callback:(void (^)(NSString *, NSError *error))callback {
    NSURL *url = [NSURL
        URLWithString:[NSString stringWithFormat:@"%@/kyouen/get?stageNo=%ld",
                                                 SERVER_DOMAIN,
                                                 (long)currentMaxStageNo]];
...
- (void)registUser:(NSString *)token
    ...
    NSString *content = [NSString
        stringWithFormat:@"token=%@&token_secret=%@", token, tokenSecret];

    NSURL *url =
        [NSURL URLWithString:[NSString stringWithFormat:@"%@/page/api_login",
                                                        SERVER_DOMAIN]];
...
- (void)addAllStageUser:(NSArray *)stages
    ...
    NSData *sendJsonData = [NSJSONSerialization dataWithJSONObject:sendJson
                                                           options:kNilOptions
                                                             error:nil];
    NSString *sendJsonStr =
        [[NSString alloc] initWithData:sendJsonData
                              encoding:NSUTF8StringEncoding];
    LOG(@"sendjson = %@", sendJsonStr);
    NSString *content = [NSString stringWithFormat:@"data=%@", sendJsonStr];

    NSURL *url =
        [NSURL URLWithString:[NSString stringWithFormat:@"%@/page/add_all",
                                                        SERVER_DOMAIN]];
    NSMutableURLRequest *request = [NSMutableURLRequest
         requestWithURL:url
            cachePolicy:NSURLRequestReloadIgnoringLocalCacheData
...
- (void)addStageUser:(NSNumber *)stageNo {
    ...
    NSURL *url =
        [NSURL URLWithString:[NSString stringWithFormat:@"%@/page/add",
                                                        SERVER_DOMAIN]];
    NSMutableURLRequest *request = [NSMutableURLRequest
         requestWithURL:url
            cachePolicy:NSURLRequestReloadIgnoringLocalCacheData
...
- (void)registDeviceToken:(NSString *)deviceToken {
    ...
    NSString *content =
        [NSString stringWithFormat:@"device_token=%@", deviceToken];

    NSURL *url =
        [NSURL URLWithString:[NSString stringWithFormat:@"%@/apns/regist",
                                                        SERVER_DOMAIN]];
    NSMutableURLRequest *request = [NSMutableURLRequest
         requestWithURL:url
            cachePolicy:NSURLRequestReloadIgnoringLocalCacheData
