...
+ (BOOL)isLocalTwitterAccountAvailable {
    ...
    available = [SLComposeViewController
...
- (void)performReverseAuthForAccount:(ACAccount *)account
                         withHandler:(TKAPIHandler)handler {
                         ...
                         if (responseData) {
                             NSString *responseStr = [[NSString alloc]
                                 initWithData:responseData
                                     encoding:NSUTF8StringEncoding];
                             NSArray *parts =
                                 [responseStr componentsSeparatedByString:@"&"];
                             ...
                             if (oauthToken && oauthTokenSecret) {
                                 // 保存する
                                 TKTwitterTokenDao *dao =
                                     [[TKTwitterTokenDao alloc] init];
                                 [dao saveToken:oauthToken
...
- (void)_step1WithCompletion:(TKAPIHandler)completion {
    ...
    TKSignedRequest *step1Request =
        [[TKSignedRequest alloc] initWithURL:url
                                  parameters:dict
                               requestMethod:TKSignedRequestMethodPOST];
...
- (void)_step2WithAccount:(ACAccount *)account
               ...
               andHandler:(TKAPIHandler)completion {
    NSDictionary *step2Params = @{
        TW_X_AUTH_REVERSE_TARGET : [TKSignedRequest consumerKey],
        TW_X_AUTH_REVERSE_PARMS : signedReverseAuthSignature
    };
    NSURL *authTokenURL = [NSURL URLWithString:TW_OAUTH_URL_AUTH_TOKEN];
    SLRequest *step2Request = [self requestWithUrl:authTokenURL
                                        parameters:step2Params
                                     requestMethod:SLRequestMethodPOST];

    [step2Request setAccount:account];
    [step2Request performRequestWithHandler:^(NSData *responseData,
