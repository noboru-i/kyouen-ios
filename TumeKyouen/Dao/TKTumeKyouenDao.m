...
- (BOOL)insertWithCsvString:(NSString *)csv {
    ...
    for (NSString *row in lines) {
        ...
        TumeKyouenModel *newObject = (TumeKyouenModel *)[NSEntityDescription
            insertNewObjectForEntityForName:@"TumeKyouenModel"
...
- (TumeKyouenModel *)selectByStageNo:(NSNumber *)stageNo {
    ...
    NSEntityDescription *entity =
        [NSEntityDescription entityForName:@"TumeKyouenModel"
                    inManagedObjectContext:self.managedObjectContext];
    [fetchRequest setEntity:entity];

    // 条件
    NSPredicate *predicate = [NSPredicate
        predicateWithFormat:@"%K = %d", @"stageNo", [stageNo intValue]];
    [fetchRequest setPredicate:predicate];

    // ソート順
    NSSortDescriptor *stageNoDescriptor =
        [[NSSortDescriptor alloc] initWithKey:@"stageNo" ascending:YES];
    NSArray *sortDescriptors =
        [[NSArray alloc] initWithObjects:stageNoDescriptor, nil];
...
- (NSUInteger)selectCountClearStage {
    ...
    NSPredicate *predicate =
        [NSPredicate predicateWithFormat:@"%K = %d", @"clearFlag", 1];
    [fetchRequest setPredicate:predicate];
...
- (void)updateSyncClearData:(NSArray *)clearStages {
    ...
    for (NSDictionary *dic in clearStages) {
        LOG(@"model = %@", dic);
        TumeKyouenModel *model =
            [self selectByStageNo:[dic objectForKey:@"stageNo"]];
        if (!model) {
            continue;
        }
        NSDate *clearDate =
            [formatter dateFromString:[dic objectForKey:@"clearDate"]];
