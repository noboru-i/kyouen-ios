...
- (void)prepareForSegue:(UIStoryboardSegue*)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"StartSegue"]) {
        ...
        TKKyouenViewController* viewController =
            (TKKyouenViewController*)[segue destinationViewController];
...
- (IBAction)connectTwitterAction:(id)sender {
    ...
    if ([self.accounts count] == 0) {
        ...
        UIAlertView* alert = [[UIAlertView alloc]
                initWithTitle:NSLocalizedString(@"alert_no_twitter_account",
                                                nil)
                      message:nil
                     delegate:nil
...
- (IBAction)syncDataAction:(id)sender {
    ...
    [server
        addAllStageUser:stages
               callback:^(NSArray* response, NSError* error) {
                 LOG_METHOD;
                 if (error) {
                     // 通信が異常終了
                     [SVProgressHUD
                         showErrorWithStatus:[error localizedDescription]];
                     return;
                 }
                 [dao updateSyncClearData:response];
                 [self refreshCounts];
                 [SVProgressHUD
                     showSuccessWithStatus:NSLocalizedString(
                                               @"progress_sync_complete", nil)];
               }];
}
    ...
    if (buttonIndex != actionSheet.cancelButtonIndex) {
        ...
        [self.twitterManager
            performReverseAuthForAccount:self.accounts[buttonIndex]
                             withHandler:^(NSData* responseData,
                                           NSError* error) {
                               if (!responseData) {
...
- (void)obtainAccessToAccountsWithBlock:(void (^)(BOOL))block {
    ...
    ACAccountStoreRequestAccessCompletionHandler handler = ^(BOOL granted,
                                                             NSError* error) {
      if (granted) {
...
- (void)refreshCounts {
    ...
    self.stageCountLabel.text =
        [NSString stringWithFormat:@"%ld/%ld", (unsigned long)clearCount,
                                   (unsigned long)allCount];
}

- (void)getStage:(NSUInteger)maxStageNo
          server:(TKTumeKyouenServer*)server
       kyouenDao:(TKTumeKyouenDao*)dao {
    LOG_METHOD;
...
- (void)sendTwitterAccount {
    ...
    [SVProgressHUD showWithMaskType:SVProgressHUDMaskTypeBlack];
    TKTumeKyouenServer* server = [[TKTumeKyouenServer alloc] init];
    [server registUser:oauthToken
           tokenSecret:oauthTokenSecret
              callback:^(NSString* response, NSError* error) {
                if (error != nil) {
                    [SVProgressHUD
                        showErrorWithStatus:NSLocalizedString(
                                                @"progress_auth_fail", nil)];
                    return;
                }
