...
- (int)getStoneCount:(int)state {
    ...
    for (int i = 0; i < [self.stage length]; i++) {
        if ([[self.stage substringWithRange:NSMakeRange(i, 1)]
                isEqualToString:stateString]) {
            count++;
        }
    }
    return count;
...
- (TKKyouenData *)isKyouen {
    ...
    if (intersection123 == nil) {
        ...
        if (intersection234 == nil) {
            // p2,p3,p4が直線状に存在する場合
            return [[TKKyouenData alloc]
                initWithLine:p1
                       point:p2
                       point:p3
                       point:p4
                        line:[[TKLine alloc] initWithPoints:p1
                                               anotherPoint:p2]];
        }
    } else {
...
- (TKLine *)getMidperpendicular:(TKPoint *)p1 anotherPoint:(TKPoint *)p2 {
    ...
    TKLine *midperpendicular =
        [[TKLine alloc] initWithPoints:midpoint
                          anotherPoint:[midpoint sum:gradient]];
